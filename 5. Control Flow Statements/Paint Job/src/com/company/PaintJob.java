...
Paint Job
        ...
        This method needs to return a value of type int that represents the number of buckets that Bob needs to
        ...
        getBucketCount(7.25, 4.3, 2.35); → should return 14 since the wall area is 31.175, and a
        single bucket can cover an area of 2.35.
        ...
        If one of the parameters area or areaPerBucket is less or equal to 0, the method needs to return -1to indicate
        an invalid value.
        ...
        NOTE: Use the method Math.ceil to round the number of calculated buckets (double) then convert it into
        an int before returning the value from the methods.
        NOTE: All methods should be defined as public static like we have been doing so far in the course.
        NOTE: Do not add the main method to the solution code.*/

public class PaintJob {
    /*Algorithm
    * Take the 4/3/2 parameters and see if they're less than 0, if yes return -1;
    * If not, calculate the areaOfWall by multiplying width into height (width * height)
    * now divide areaOfWall by areaPerBucket, round it upwards and store value in var bucketsNeeded.
    * Subtract extra buckets from bucketsNeeded, if value is negative return 0 else return the subtracted amount*/
...
//    OverLoaded Method which doesn't use extraBuckets Parameter;
...
//    Last Overloaded method which calculates bucketsNeeded by just using wallarea and bucket area
    public static int getBucketCount(double areaOfWall, double areaPerBucket){
        if(areaOfWall < 1 || areaPerBucket < 1 ){
            return -1;
        }
        return (int) (Math.ceil(areaOfWall/areaPerBucket));
    }
